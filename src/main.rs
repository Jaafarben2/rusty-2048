...
use crate::game::game_variants::game_variant_0::{GameVariant as GameVariant0, AllowedMoves as AllowedMoves0, GameStatus as GameStatus0};
use crate::game::game_variants::game_variant_1::{GameVariant as GameVariant1, AllowedMoves as AllowedMoves1, GameStatus as GameStatus1, SpecificElementType};
            ...
            div { class: "pt-20 max-w-6xl mx-auto px-6 min-h-[100dvh]",
                ...
                section { class: "grid gap-8 sm:grid-cols-2 lg:grid-cols-3",
                    ...
                    Link {
                        ...
                        p { class: "text-sm text-[#776e65] mt-2",
                            "The original 4×4 grid variant 1. Combine tiles to reach 2048!"
                        }
                    }
...
pub fn Classic2048Inner() -> Element {
    ...
    rsx! {
        ...
        div {
            ...
            onkeydown: move |evt| {
                match evt.key() {
                    Key::ArrowLeft => g_signal.write().move_generic(AllowedMoves0::LEFT),
                    Key::ArrowRight => g_signal.write().move_generic(AllowedMoves0::RIGHT),
                    Key::ArrowUp => g_signal.write().move_generic(AllowedMoves0::UP),
                    Key::ArrowDown => g_signal.write().move_generic(AllowedMoves0::DOWN),

                    _ => {}
                }
                let status = g_signal.read().game_status;
                //check game status if end to show game over
                if status == GameStatus0::END_FAIL {
                    logs.write().push("Game Over".to_string());

                } else if status == GameStatus0::END_SUCCESS {
            ...
            ontouchend: move |evt| {
                ...
                if directions.len() == 1 {
                    match directions[0] {
                        Direction::Left => g_signal.write().move_generic(AllowedMoves0::LEFT),
                        Direction::Right => g_signal.write().move_generic(AllowedMoves0::RIGHT),
                        Direction::Down => g_signal.write().move_generic(AllowedMoves0::DOWN),
                        Direction::Up => g_signal.write().move_generic(AllowedMoves0::UP),
                    }
            ...
            // Game END
            if g_signal.read().game_status == GameStatus0::END_FAIL
                || g_signal.read().game_status == GameStatus0::END_SUCCESS
            {
                div { class: "absolute inset-0 bg-black/40 rounded-lg flex items-center justify-center",
                    div { class: "bg-[#faf8ef] p-6 rounded-lg shadow-xl text-center w-[90%] max-w-xs",
                        ...
                        } else if g_signal.read().game_status == GameStatus0::END_SUCCESS {
...
#[component]
pub fn Classic2048InnerVariant1() -> Element {
    ...
    const c_width : usize = 4;
    const c_height : usize = 4;
    let mut g = GameVariant1::<c_width, c_height>::new_game();
    let mut g_signal = use_signal(move || g);
    ...
    fn get_class_x(el: Option<SpecificElementType>) -> &'static str {
        match el {
            ...
            Some(SpecificElementType::Block) => "aspect-square bg-black rounded",
    ...
    rsx! {
        ...
        div {
            ...
            onkeydown: move |evt| {
                match evt.key() {
                    Key::ArrowLeft => g_signal.write().move_generic(AllowedMoves1::LEFT),
                    Key::ArrowRight => g_signal.write().move_generic(AllowedMoves1::RIGHT),
                    Key::ArrowUp => g_signal.write().move_generic(AllowedMoves1::UP),
                    Key::ArrowDown => g_signal.write().move_generic(AllowedMoves1::DOWN),

                    _ => {}
                }
                let status = g_signal.read().game_status;
                //check game status if end to show game over
                if status == GameStatus1::END_FAIL {
                    logs.write().push("Game Over".to_string());

                } else if status == GameStatus1::END_SUCCESS {
                    logs.write().push("You Win!".to_string());
                }
            },
            ...
            ontouchend: move |evt| {
                ...
                if directions.len() == 1 {
                    match directions[0] {
                        Direction::Left => g_signal.write().move_generic(AllowedMoves1::LEFT),
                        Direction::Right => g_signal.write().move_generic(AllowedMoves1::RIGHT),
                        Direction::Down => g_signal.write().move_generic(AllowedMoves1::DOWN),
                        Direction::Up => g_signal.write().move_generic(AllowedMoves1::UP),
                    }
            ...
            div { class: "wrapper",
                div { class: "mb-4 flex justify-between",
                    ...
                    button {
                        class: "px-4 py-2 bg-[#8f7a66] text-white rounded hover:bg-[#7c6957]",
                        onclick: move |_| {
                            *g_signal.write() = GameVariant1::<c_width, c_height>::new_game();
                        },
                        "Restart"
                    }
                    div {
                                ...
                                div { class: get_class_x(g_signal.read().game_variant_data.array[i][j]),
                                    {
                                        g_signal
                                            ...
                                            .array[i][j]
                                            .map(|v| match v {
                                                SpecificElementType::Some(num) => num.to_string(),
                                                _ => String::from("b"),
                                            })
                                            .unwrap_or_default()
                                    }
            ...
            // Game END
            if g_signal.read().game_status == GameStatus1::END_FAIL
                || g_signal.read().game_status == GameStatus1::END_SUCCESS
            {
                div { class: "absolute inset-0 bg-black/40 rounded-lg flex items-center justify-center",
                    div { class: "bg-[#faf8ef] p-6 rounded-lg shadow-xl text-center w-[90%] max-w-xs",
                        if g_signal.read().game_status == GameStatus1::END_FAIL {
                            h2 { class: "text-3xl font-bold text-[#776e65] mb-4", "Game Over" }
                            p { class: "text-[#776e65] mb-6", "No more moves left!" }
                        } else if g_signal.read().game_status == GameStatus1::END_SUCCESS {
                            h2 { class: "text-3xl font-bold text-[#776e65] mb-4", "You Win!" }
                        }
                        div { class: "flex gap-3 justify-center",
                            button {
